<template>
  <div class="pagination-component">
    <ul v-show="totalPages > 1" class="pagination">
      ...
      <li :class="'page-item ' + allowedPageClass(page + 1)">
        <a class="page-link" href="javascript:void(0)" @click="next">Next | <span v-show="parseInt(items)">{{count}}</span></a>
      </li>
...
export default {
  props: {
    ...
    countText: {
      type: String,
      default: 'Showing {from} to {to} of {count} results|{count} results|1 result'
    },
    currentPage: {
      type: Number,
      default: 1
